//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//
//  Build 2022-10-30 xybp888 Dump.
//  

#import <SpringBoardUIServices/SBUIRemoteAlertServiceViewController.h>

@class LACachedExternalizedContext, NSDictionary, NSString, _TtC10CoreAuthUI19TransitionViewModel;
@protocol LABackoffCounter, LARemoteUIHost, LAUIMechanism;

@interface TransitionViewController : SBUIRemoteAlertServiceViewController
{
    TransitionViewController *_childController;
    long long _resetUiEvent;
    _Bool _allowsLandscape;
    _Bool _hideStatusBar;
    long long _presentationMode;
    _TtC10CoreAuthUI19TransitionViewModel *_viewModel;
    _Bool _callerNameOverride;
    _Bool _awaitingDissappear;
    _Bool _disappeared;
    _Bool _appeared;
    unsigned int _instanceId;
    id <LAUIMechanism> _mechanism;
    id <LARemoteUIHost> _remoteUIHost;
    long long _policy;
    NSDictionary *_options;
    id <LABackoffCounter> _backoffCounter;
    NSString *_callerName;
    NSString *_callerBundleId;
    LACachedExternalizedContext *_cachedExternalizedContext;
    NSDictionary *_internalInfo;
    NSString *_authenticationTitle;
    NSString *_authenticationSubtitle;
    CDUnknownBlockType _disappearedNotification;
    CDUnknownBlockType _appearedNotification;
}

+ (id)rootController;
+ (unsigned int)newTaskId;
+ (unsigned int)newInstanceId;
+ (_Bool)_isSecureForRemoteViewService;
- (void).cxx_destruct;
@property(readonly, nonatomic) unsigned int instanceId; // @synthesize instanceId=_instanceId;
@property(copy, nonatomic) CDUnknownBlockType appearedNotification; // @synthesize appearedNotification=_appearedNotification;
@property(copy, nonatomic) CDUnknownBlockType disappearedNotification; // @synthesize disappearedNotification=_disappearedNotification;
@property(nonatomic) _Bool appeared; // @synthesize appeared=_appeared;
@property(nonatomic) _Bool disappeared; // @synthesize disappeared=_disappeared;
@property(nonatomic) _Bool awaitingDissappear; // @synthesize awaitingDissappear=_awaitingDissappear;
@property(retain, nonatomic) NSString *authenticationSubtitle; // @synthesize authenticationSubtitle=_authenticationSubtitle;
@property(retain, nonatomic) NSString *authenticationTitle; // @synthesize authenticationTitle=_authenticationTitle;
@property(readonly, nonatomic) NSDictionary *internalInfo; // @synthesize internalInfo=_internalInfo;
@property(readonly, nonatomic) LACachedExternalizedContext *cachedExternalizedContext; // @synthesize cachedExternalizedContext=_cachedExternalizedContext;
@property(readonly, nonatomic) NSString *callerBundleId; // @synthesize callerBundleId=_callerBundleId;
@property(readonly, nonatomic) _Bool callerNameOverride; // @synthesize callerNameOverride=_callerNameOverride;
@property(readonly, nonatomic) NSString *callerName; // @synthesize callerName=_callerName;
@property(retain, nonatomic) id <LABackoffCounter> backoffCounter; // @synthesize backoffCounter=_backoffCounter;
@property(readonly, nonatomic) NSDictionary *options; // @synthesize options=_options;
@property(readonly, nonatomic) long long policy; // @synthesize policy=_policy;
@property(readonly, nonatomic) id <LARemoteUIHost> remoteUIHost; // @synthesize remoteUIHost=_remoteUIHost;
@property(readonly, nonatomic) id <LAUIMechanism> mechanism; // @synthesize mechanism=_mechanism;
- (void)viewModelDidReceiveAuthenticationDataWithInternalInfo:(id)arg1 mechanism:(id)arg2;
- (void)viewModelDidReceiveMechanismEvent:(long long)arg1 value:(id)arg2 reply:(CDUnknownBlockType)arg3;
- (void)viewModelDidReceiveMechanismEvent:(long long)arg1 reply:(CDUnknownBlockType)arg2;
- (void)viewModelDidReceiveDismissWasInvalidated:(_Bool)arg1 completionHandler:(CDUnknownBlockType)arg2;
- (void)showWithController:(long long)arg1 internalInfo:(id)arg2 completionHandler:(CDUnknownBlockType)arg3;
- (_Bool)_canShowWhileLocked;
- (_Bool)_internalBooleanDefaultNamed:(id)arg1;
- (_Bool)_shouldSimulateStaleRootController;
- (_Bool)_shouldSkipStaleRootControllerRecovery;
- (void)_dismiss;
- (void)noteActivatedWithPresentationMode:(long long)arg1;
- (void)configureWithContext:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (long long)preferredStatusBarStyle;
- (int)_preferredStatusBarVisibility;
- (unsigned long long)supportedInterfaceOrientations;
- (_Bool)processHomeButtonPressed;
- (void)handleButtonActions:(id)arg1;
- (void)dismissChildWithCompletionHandler:(CDUnknownBlockType)arg1;
- (void)mechanismEvent:(long long)arg1 value:(id)arg2 reply:(CDUnknownBlockType)arg3;
- (void)mechanismEvent:(long long)arg1 reply:(CDUnknownBlockType)arg2;
- (void)internalInfoWithReply:(CDUnknownBlockType)arg1;
- (void)_setActivityWatchdog;
- (void)authMethodWithReply:(CDUnknownBlockType)arg1;
- (void)externalizedContextWithReply:(CDUnknownBlockType)arg1;
- (void)uiFailureWithError:(id)arg1;
- (void)uiSuccessWithResult:(id)arg1;
- (void)uiFallback;
- (void)uiCancel;
- (void)uiEvent:(long long)arg1 options:(id)arg2;
- (_Bool)isRemoteViewController;
- (_Bool)isFullScreenController;
- (void)_scheduleIdleExit;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)idleTimerDisable:(_Bool)arg1;
- (_Bool)prefersStatusBarHidden;
- (void)viewWillAppear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewDidLoad;
- (_Bool)_shouldHideStatusBarForController:(long long)arg1;
- (void)_performOnMainQueueWhenAppeared:(CDUnknownBlockType)arg1;
- (_Bool)shouldPresentWithAnimation;
- (void)transitionToController:(long long)arg1 internalInfo:(id)arg2 completionHandler:(CDUnknownBlockType)arg3;
- (void)dismissRemoteUIWasInvalidated:(_Bool)arg1 completionHandler:(CDUnknownBlockType)arg2;
- (void)_dismissRemoteUIWithCompletionHandler:(CDUnknownBlockType)arg1;
- (id)localizedCallerName;
@property(readonly, copy) NSString *description;
- (void)didReceiveAuthenticationData;
- (id)initWithInternalInfo:(id)arg1 mechanism:(id)arg2 backoffCounter:(id)arg3 remoteUIHost:(id)arg4 allowsLandscape:(_Bool)arg5;
- (id)initWithInternalInfo:(id)arg1 parent:(id)arg2 allowsLandscape:(_Bool)arg3;
- (void)_setupConnection;
- (void)_resetUI;
- (_Bool)_isRootControllerClass;
- (void)dealloc;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

