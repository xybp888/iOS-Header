//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//
//  Build 2022-10-30 xybp888 Dump.
//  

@class NSMutableArray, TSCH3DBarChartDefaultAppearance, TSCH3DLabelResources;
@protocol TSCH3DBarChartAppearance;

@interface TSCH3DChartBarElementProperties
{
    struct vector<float, std::allocator<float>> _maxValues;
    struct vector<TSCH3D::BarElementInfo, std::allocator<TSCH3D::BarElementInfo>> _elementInfos;
    _Bool _stacked;
    _Bool _bevelEdges;
    float _barWidth;
    float _elementsXOffset;
    float _seriesOffset;
    float _setWidth;
    int _barShape;
    TSCH3DLabelResources *_seriesLabels;
    struct array<TSCH3D::BarExtrusionDetails, 2UL> _extrusionDetails;
    TSCH3DBarChartDefaultAppearance *_appearance;
    NSMutableArray *_rangeCache;
}

- (id).cxx_construct;
- (void).cxx_destruct;
@property(readonly, nonatomic) _Bool hasBevelEdges; // @synthesize hasBevelEdges=_bevelEdges;
@property(readonly, nonatomic) int barShape; // @synthesize barShape=_barShape;
@property(retain, nonatomic) TSCH3DLabelResources *seriesLabels; // @synthesize seriesLabels=_seriesLabels;
@property(readonly, nonatomic) id <TSCH3DBarChartAppearance> appearance;
- (id)p_appearance;
- (void)releaseAndClearAppearance;
- (void)p_createAppearance;
- (void)p_updateAppearance;
- (void)p_resetExtrusionDetails;
- (id)p_appearanceClasses;
- (void)setExtrusionDetails:(const struct BarExtrusionDetails *)arg1 forShape:(int)arg2;
- (const struct BarExtrusionDetails *)extrusionDetailsForShape:(int)arg1;
- (float)maxValueForSeries:(long long)arg1;
- (void)reset;
- (float)chartInitialDepthOffset;
- (void)updateLabels;
- (void)createResources;
- (void)updateMaxValuesAndBevels;
- (void)calculateLayout;
- (id)texcoordsForSeries:(id)arg1 index:(const void *)arg2;
- (id)normalsForSeries:(id)arg1 index:(const void *)arg2;
- (id)boundsGeometryForSeries:(id)arg1 index:(const void *)arg2;
- (id)geometryForSeries:(id)arg1 index:(const void *)arg2;
- (id)p_barResourceCacheItemAtIndex:(const void *)arg1;
- (_Bool)applyElementTransform:(void *)arg1 series:(id)arg2 index:(tvec2_3b141483)arg3 propertyAccessor:(id)arg4;
- (float)elementTransformDepthFromPropertyAccessor:(id)arg1;
- (float)barWidth;
- (float)interceptValueForSeries:(id)arg1;
- (id)p_calculateRangeForSeries:(id)arg1 index:(void *)arg2 intercept:(id)arg3;
- (id)rangeForSeries:(id)arg1 index:(void *)arg2;
- (void)p_updateRangeCache;
- (id)calculateInterceptForSeries:(id)arg1;
- (_Bool)beveledAtIndex:(const void *)arg1;
- (void)setBeveledAtIndex:(const void *)arg1;
- (void)setMaxValueForSeries:(long long)arg1 value:(float)arg2;
- (struct BarElementInfo *)elementInfoAtIndex:(const void *)arg1;
- (struct BarElementInfo)barElementInfoAtIndex:(const void *)arg1;
- (void)setElementInfo:(const struct BarElementInfo *)arg1 atIndex:(const void *)arg2;
- (long long)flatIndex:(void *)arg1;
- (void)dealloc;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)init;

@end

