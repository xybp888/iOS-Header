//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//
//  Build 2022-10-30 xybp888 Dump.
//  

#import <objc/NSObject.h>

@class NSMutableArray, NSString, SUAnalyticsManager, SUAutoInstallManager, SUSFollowUpController;
@protocol OS_dispatch_queue, SUManagerInterface;

@interface SUManagerServer : NSObject
{
    SUAutoInstallManager *_autoInstallManager;
    SUAnalyticsManager *_coreAnalyticsManager;
    id <SUManagerInterface> _manager;
    NSMutableArray *_clients;
    SUSFollowUpController *_followUpController;
    NSObject<OS_dispatch_queue> *_clientQueue;
    _Bool _underExclusiveControl;
}

+ (id)sharedInstance;
@property _Bool underExclusiveControl; // @synthesize underExclusiveControl=_underExclusiveControl;
- (void)autoInstallManager:(id)arg1 passcodePolicyChanged:(unsigned long long)arg2 forOperation:(id)arg3;
- (void)autoInstallManager:(id)arg1 isReadyToInstall:(id)arg2;
- (void)autoInstallManager:(id)arg1 didExpireOperation:(id)arg2 withError:(id)arg3;
- (void)autoInstallManager:(id)arg1 didCancelOperation:(id)arg2;
- (void)autoInstallManager:(id)arg1 operationWasConsented:(id)arg2;
- (void)downloadWasInvalidated:(id)arg1;
- (void)rollbackReadyForReboot:(id)arg1;
- (void)rollbackSucceeded:(id)arg1;
- (void)rollbackDidFail:(id)arg1 withError:(id)arg2;
- (void)rollbackDidStart:(id)arg1;
- (void)rollbackReadyToStart:(id)arg1 options:(id)arg2 completion:(CDUnknownBlockType)arg3;
- (void)installTonightScheduled:(_Bool)arg1;
- (void)splatRollbackDetected:(id)arg1;
- (void)splatUpdateDetected;
- (void)newOSDetected:(id)arg1;
- (void)managedInstallationRequested:(id)arg1;
- (void)installPolicyDidChange:(id)arg1;
- (void)installDidFinish:(id)arg1;
- (void)installDidFail:(id)arg1 withError:(id)arg2;
- (void)installDidStart:(id)arg1;
- (void)clearingSpaceForDownload:(id)arg1 clearing:(_Bool)arg2;
- (void)downloadWasInvalidatedForNewUpdateAvailable:(id)arg1;
- (void)downloadWasInvalidatedForNewUpdatesAvailable:(id)arg1;
- (void)downloadDidFinish:(id)arg1 withInstallPolicy:(id)arg2;
- (void)downloadDidFinish:(id)arg1;
- (void)downloadDidFail:(id)arg1 withError:(id)arg2;
- (void)downloadProgressDidChange:(id)arg1;
- (void)downloadDidStart:(id)arg1;
- (void)automaticDownloadDidFailToStartForNewUpdateAvailable:(id)arg1 withError:(id)arg2;
- (void)scanDidCompleteWithNewUpdateAvailable:(id)arg1 error:(id)arg2;
- (void)scanRequestDidFinishForOptions:(id)arg1 update:(id)arg2 error:(id)arg3;
- (void)scanDidCompleteForOptions:(id)arg1 results:(id)arg2 error:(id)arg3;
- (void)scanRequestDidFinishForOptions:(id)arg1 results:(id)arg2 error:(id)arg3;
- (void)scanRequestDidStartForOptions:(id)arg1;
- (void)setClientType:(int)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)removeClient:(id)arg1;
- (void)addClient:(id)arg1;
- (void)exitExclusiveControl;
- (void)goUnderExclusiveControl;
- (id)clients;
- (void)_clientForegroundnessDidChange:(id)arg1;
- (void)_evaluateForegroundness;
- (void)_clientMessagabilityDidChange:(id)arg1;
- (void)_sendLatestStatusForClient:(id)arg1;
- (void)submitSUAnalyticsEventsWithName:(id)arg1;
- (void)submitAllSUAnalyticsEvents;
- (void)submitSUAnalyticsEvent:(id)arg1;
- (void)recordSUAnalyticsEvent:(id)arg1;
- (void)writeKeepAliveFile:(CDUnknownBlockType)arg1;
- (void)setExclusiveControl:(_Bool)arg1;
- (void)installServerConfiguration;
- (void)destroyInstallationKeybag;
- (void)createInstallationKeybag:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)securityResponseRollbackSuggested:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)setLastRollbackDescriptor:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)scheduleRollbackRebootForLater:(CDUnknownBlockType)arg1;
- (void)isRollingBack:(CDUnknownBlockType)arg1;
- (void)previousRollback:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)rollbackUpdateWithOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)eligibleRollbackWithOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)presentAutoUpdateBanner:(CDUnknownBlockType)arg1;
- (void)registerCSInstallPredicatesOnDate:(id)arg1;
- (void)isAutomaticDownloadEnabled:(CDUnknownBlockType)arg1;
- (void)enableAutomaticDownload:(_Bool)arg1;
- (void)isAutomaticUpdateV2Enabled:(CDUnknownBlockType)arg1;
- (void)enableAutomaticUpdateV2:(_Bool)arg1;
- (void)isAutoUpdateScheduled:(CDUnknownBlockType)arg1;
- (void)isAutoUpdateEnabled:(CDUnknownBlockType)arg1;
- (void)currentPasscodePolicy:(CDUnknownBlockType)arg1;
- (void)consentToAutoInstallOperation:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)cancelAutoInstallOperation:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)currentAutoInstallOperation:(_Bool)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)getMandatorySoftwareUpdateDictionary:(CDUnknownBlockType)arg1;
- (void)setMandatorySoftwareUpdateDictionary:(id)arg1;
- (void)delayEndDate:(CDUnknownBlockType)arg1;
- (void)isDelayingUpdates:(CDUnknownBlockType)arg1;
- (void)softwareUpdatePathRestriction:(CDUnknownBlockType)arg1;
- (void)slaVersion:(CDUnknownBlockType)arg1;
- (void)installUpdateWithInstallOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)installUpdateWithOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)installUpdate:(CDUnknownBlockType)arg1;
- (void)isUpdateReadyForInstallationWithOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)isUpdateReadyForInstallation:(CDUnknownBlockType)arg1;
- (void)isInstallationKeybagRequiredForDescriptor:(id)arg1 result:(CDUnknownBlockType)arg2;
- (void)isInstallationKeybagRequired:(CDUnknownBlockType)arg1;
- (void)deviceHasSufficientSpaceForDownloadIncludeAppPurging:(_Bool)arg1 descriptor:(id)arg2 replyHandler:(CDUnknownBlockType)arg3;
- (void)deviceHasSufficientSpaceForDownloadIncludeAppPurging:(_Bool)arg1 replyHandler:(CDUnknownBlockType)arg2;
- (void)deviceHasSufficientSpaceForDownloadsIncludeAppPurging:(_Bool)arg1 replyHandler:(CDUnknownBlockType)arg2;
- (void)extraSpaceNeededForDownloadWithoutAppPurging:(CDUnknownBlockType)arg1;
- (void)deviceHasSufficientSpaceForDownload:(CDUnknownBlockType)arg1;
- (void)deviceHasSufficientSpaceForDownloads:(CDUnknownBlockType)arg1;
- (void)updatesDownloadableWithOptions:(id)arg1 alternateDownloadOptions:(id)arg2 replyHandler:(CDUnknownBlockType)arg3;
- (void)downloadAndInstallState:(CDUnknownBlockType)arg1;
- (void)updateDownloadOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)updateDownloadMetadata:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)resumeDownload:(CDUnknownBlockType)arg1;
- (void)pauseDownload:(CDUnknownBlockType)arg1;
- (void)purgeDownloadWithOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)cancelDownloadWithOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)startDownloadWithMetadata:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)startDownloadWithOptions:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)startDownload:(CDUnknownBlockType)arg1;
- (void)isDownloading:(CDUnknownBlockType)arg1;
- (void)isClearingSpaceForDownload:(CDUnknownBlockType)arg1;
- (void)descriptor:(CDUnknownBlockType)arg1;
- (void)descriptors:(CDUnknownBlockType)arg1;
- (void)autoScanAndDownloadIfAvailable:(CDUnknownBlockType)arg1;
- (void)scanForUpdates:(id)arg1 withResult:(CDUnknownBlockType)arg2;
- (void)isScanning:(CDUnknownBlockType)arg1;
- (void)handleClientRequest:(const char *)arg1 client:(id)arg2 withRequestCallBack:(CDUnknownBlockType)arg3 withErrorCallBack:(CDUnknownBlockType)arg4;
- (void)runOnClientsUntilStop:(CDUnknownBlockType)arg1;
- (void)runOnClients:(CDUnknownBlockType)arg1;
- (void);
- (id)coreAnalyticsManager;
- (id)autoInstallManager;
- (id)manager;
- (id)init;
- (void)_notifyServerStart;
- (_Bool)listener:(id)arg1 shouldAcceptNewConnection:(id)arg2;
- (id)_clientForCurrentConnection;
- (void)_run;
- (void)runUntilIdleExit;
- (void)serverInitAndResumeWork;
- (id)connectionQueue;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

