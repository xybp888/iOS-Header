//
//     Generated by class-dump 3.5 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import "NSCopying-Protocol.h"

@class NSString, NSURL;

@interface MCMUserIdentity : NSObject <NSCopying>
{
    NSURL *_homeDirectoryURL;
    unsigned int _kernelPersonaID;
    unsigned int _posixUID;
    unsigned int _posixGID;
    int _personaType;
    struct NSString *_personaUniqueString;
    NSString *_identifier;
}

+ (id)_identifierForPOSIXUID:(unsigned int)arg1 personaUniqueString:(struct NSString *)arg2 personaType:(int)arg3;
+ (_Bool)isUserIdentityRequiredForContainerClass:(unsigned long long)arg1;
+ (id)userIdentityWithPlist:(id)arg1 cache:(id)arg2 error:(unsigned long long *)arg3;
@property(readonly, nonatomic) int personaType; // @synthesize personaType=_personaType;
@property(readonly, nonatomic) NSURL *homeDirectoryURL; // @synthesize homeDirectoryURL=_homeDirectoryURL;
@property(readonly, nonatomic) unsigned int posixGID; // @synthesize posixGID=_posixGID;
@property(readonly, nonatomic) unsigned int posixUID; // @synthesize posixUID=_posixUID;
@property(readonly, nonatomic) NSString *identifier; // @synthesize identifier=_identifier;
@property(readonly, nonatomic) unsigned int kernelPersonaID; // @synthesize kernelPersonaID=_kernelPersonaID;
@property(readonly, nonatomic) NSString *personaUniqueString; // @synthesize personaUniqueString=_personaUniqueString;
- (void).cxx_destruct;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (_Bool)isEqual:(id)arg1;
- (_Bool)isEqualToUserIdentity:(id)arg1;
- (unsigned long long)hash;
- (id)_descriptionForPersonaType:(int)arg1;
@property(readonly, nonatomic) NSString *shortDescription;
- (id)description;
- (unsigned int)posixGIDForContainerClass:(unsigned long long)arg1;
- (unsigned int)posixUIDForContainerClass:(unsigned long long)arg1;
@property(readonly, nonatomic, getter=isNoSpecificPersona) _Bool noSpecificPersona;
@property(readonly, nonatomic, getter=isEnterprise) _Bool enterprise;
@property(readonly, nonatomic, getter=isHomeDirectoryInCommonSandbox) _Bool homeDirectoryInCommonSandbox;
@property(readonly, copy, nonatomic) id plist;
- (id)initWithVersion2PlistDictionary:(id)arg1 cache:(id)arg2 error:(unsigned long long *)arg3;
- (id)initWithVersion1PlistDictionary:(id)arg1 cache:(id)arg2 error:(unsigned long long *)arg3;
- (id)initWithPlist:(id)arg1 cache:(id)arg2 error:(unsigned long long *)arg3;
- (id)initWithUID:(unsigned int)arg1 GID:(unsigned int)arg2 homeDirectoryURL:(id)arg3 personaUniqueString:(struct NSString *)arg4 personaType:(int)arg5 kernelPersonaID:(unsigned int)arg6;
- (id)init;

@end

